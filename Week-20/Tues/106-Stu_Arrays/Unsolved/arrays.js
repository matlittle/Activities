...
princesses.forEach( el => console.log(`${el.name}: ${el.age}`) );


// create an array of princess names from the existing array
// can be replaced with map
 let names = princesses.map(el => el.name);
/* for (let i = 0; i < princesses.length; i++) {
...
let mNames = princesses.filter(el => el.name[0] === "M");
...
/* let sum = princesses.reduce( (val, el) => { val = ((val+el) / 2) } );
for (let i = 0; i < princesses.length; i++) {
  sum += princesses[i].age;
} */
const average = princesses.reduce( (val, el) => ((val+el.age) / 2), 0 );
console.log("average age: ", average);


// BONUS: get the average age of all princesses whose name includes an 'l'
let lNames = princesses
              .filter(el => el.name.indexOf('l') >= 0)
              .reduce( (val, el) => ((val+el.age) / 2), 0);
/* for (let i = 0; i < princesses.length; i++) {
